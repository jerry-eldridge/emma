...
int main(int argc, char** argv)
{
	...
	int w, h, maxval;
	Read_PPM_File(imgfile,maxval,w,h,g_img); // uses Emma
	gw = w; gh = h;
...
{
	Sleep(15);
	color *img = new color[gw*gh];
	int i,j;
	int vel = 1;
	for (i=0; i<gw*gh; i++) img[i] = g_img[(gw*gh+i-vel*gh)%(gw*gh)];
	for (i=0; i<gw*gh; i++) g_img[i] = img[i];
