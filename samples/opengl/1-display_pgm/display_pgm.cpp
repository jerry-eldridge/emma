//
// emma-image.cpp - displays a PGM image using glew/OpenGL/glut.
//    (It uses Emma graphics library to read in PGM image. Edit
//     PGM image to view contents.) Alternatively, there are
//    image editors that read PGM files, eg Irfanview, Adobe.
//
#include <windows.h>
...
void ReadTexture(GLubyte *cimg, int *img, int w, int h);
...
{
	if (argc < 2) return 1;
	char imgfile[4096] = "graph-215.pgm";
	int w, h, maxval;
	int *img = 0;
	Read_PGM_File(argv[1],maxval,w,h,img); // uses Emma
...
void ReadTexture(GLubyte *cimg, int *img, int w, int h)
{
	...
	for (j=0; j<cH; j++) {
		for (i=0; i<cW; i++) {
			c = img[j+h*i]; // make monochrome
			cImage[(i + w*j)*3 + 0] = c; // red
			cImage[(i + w*j)*3 + 1] = c; // green
			cImage[(i + w*j)*3 + 2] = c; // blue
		}
	}
